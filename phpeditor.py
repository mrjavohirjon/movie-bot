...
async def start(client, msg):
    ...
    if not await joined(client, msg.from_user.id):
        await msg.reply("⚠ Please join channel first:", reply_markup=join_btn())
        return
...
async def top(client, cb):
    ...
    movies = load(MOVIES_FILE)
    top = sorted(movies, key=lambda x: x["downloads"], reverse=True)[:5]
...
async def view_requests(client, cb):
