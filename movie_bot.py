...
BOT_TOKEN = "8509897503:AAF1GJNjEe6CC88QSXZbBIYbSTXSfQ1IJCU"
...
async def search(client,msg):
    ...
    if found:
        await client.send_video(
            msg.chat.id,
            found["file_id"],
            caption=f"🎬 {found['title']}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⭐ Add Favorite", callback_data=f"fav_{found['code']}")]
            ])
        )

        found["downloads"] = found.get("downloads",0) + 1
        save(MOVIES_FILE,movies)
...
async def stats(client,cb):
    ...
    await cb.message.edit_text(
        ...
        f"👥 Users: {len(users)}\n"
        f"🎬 Movies: {len(movies)}\n"
        f"⬇ Downloads: {sum(m.get('downloads',0) for m in movies)}",
